...
app.post('/api/clientes', async (request, response) => {
  const { nombre, identificacion, telefono, correo, ciudad, direccion, notas } = request.body;
  if (!nombre || !telefono) return response.status(400).json({ message: 'Nombre y teléfono son obligatorios.' });
  try {
    const [result] = await pool.execute(
      'INSERT INTO clientes (nombre, identificacion, telefono, correo, ciudad, direccion, notas) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [nombre, identificacion || null, telefono, correo || null, ciudad || null, direccion || null, notas || null]
    );
    response.status(201).json({ id: result.insertId, message: 'Cliente guardado correctamente.' });
  } catch (_error) {
    response.status(500).json({ message: 'No fue posible guardar el cliente.' });
  }
});
